...
use crate::spider_well::environment::LEVEL_DEPTH;
...
impl Stage2 {
    ...
    fn spawn_blocks_1(
        ...
        for ways in [ways_a, ways_b] {
            commands.spawn((
                Mesh3d(assets.cube.clone()),
                MeshMaterial3d(assets.black_mat.clone()),
                Transform::from_xyz(0.0, -Self::START, 0.0)
                    .with_scale(Vec3::new(Self::BLOCKS1WIDTH, Self::BLOCKS1HEIGHT, 1.0)),
...
pub fn spawn_stage_2(
    ...
    let vec = Stage2::spawn_blocks_2(&mut commands, &assets);
    commands.insert_resource(SlidingBlocks2{vec});
}